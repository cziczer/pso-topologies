...
class PSOFreeScale(PSO):
    def __init__(self, num_city, data, neighbours_radius, max_iter=20_000, node_degree=11):
    ...
    def create_neighbours_topology(self):
        """
        Creates a free-scale network topology using the Barabási-Albert model.
        """
        # Use NetworkX to generate a scale-free graph
        scale_free_graph = nx.barabasi_albert_graph(self.num, 11) #elf.node_degree)  # m=2: Each new node connects to 2 existing nodes
        neighbor_dict = {}

        for node in scale_free_graph.nodes:
            neighbor_dict[node] = list(scale_free_graph.neighbors(node))

        return neighbor_dict

    def get_neighbours_best(self, start_particle_idx):
        """
        Finds the best (minimum-length) neighbor for a given particle based on the free-scale topology,
        ...
        def bst_traverse(node_idx, depth, visited):
            ...
            neighbors = self.neighbours.get(node_idx, [])
